...
class ForgeSchema extends Schema
{
    ...
    ForgeSchema(final int versionKey, final Schema parent, final Schema wrapped)
    {
        ...
        {
            RECURSIVE_TYPES = Maps.newConcurrentMap();
        }

        if (TYPE_TEMPLATES == null)
        {
            TYPE_TEMPLATES = Maps.newConcurrentMap();
        }
    ...
    {
        this.fixerFactories.addAll(fixerFactories);
    }

    public List<DataFix> getCustomFixers()
    {
        return this.fixerFactories.stream().map(factory -> factory.apply(this)).collect(Collectors.toList());
    }
}
