...
public class AdvancedParticleManager extends ParticleManager
{
    ...
    public List<AdvancedParticleDigging> addBlockDestroyEffects(BlockPos pos, TextureAtlasSprite icon)
    {
        List<AdvancedParticleDigging> particles = new ArrayList<AdvancedParticleDigging>();
        int i = 4;
        for (int j = 0; j < i; ++j)
        {
            for (int k = 0; k < i; ++k)
            {
                for (int l = 0; l < i; ++l)
                {
                    ...
                    AdvancedParticleDigging particle = new AdvancedParticleDigging(this.worldObj, d0, d1, d2, d0 - pos.getX() - 0.5D,
                            d1 - pos.getY() - 0.5D, d2 - pos.getZ() - 0.5D, icon);
                    this.addEffect(particle.setBlockPos(pos));
    ...
    public AdvancedParticleDigging addBlockHitEffects(BlockPos pos, EnumFacing side, AxisAlignedBB box, TextureAtlasSprite icon)
    {
        int i = pos.getX();
        int j = pos.getY();
        int k = pos.getZ();
        ...
        AdvancedParticleDigging particle = new AdvancedParticleDigging(this.worldObj, d0, d1, d2, 0.0D, 0.0D, 0.0D, icon);
        this.addEffect(particle.setBlockPos(pos).multiplyVelocity(0.2F).multipleParticleScaleBy(0.6F));
        return particle;
    }
