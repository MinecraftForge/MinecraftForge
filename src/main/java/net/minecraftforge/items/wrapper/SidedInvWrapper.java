...
public class SidedInvWrapper implements IItemHandler {
	...
	public static ItemStack insertItem(ISidedInventory inv, int s, ItemStack insertStack, EnumFacing side, boolean simulate) {
		...
		if (insertStack == null) return null;

		int slot = getSlot(inv, s, side);

		if (slot == -1) return insertStack;
		...
		int m;
		if (stackInSlot != null) {
			if (!ItemHandlerHelper.canItemsStack(insertStack, stackInSlot))
				return insertStack;
			...
			if (m < insertStack.stackSize) {
				...
				if (!simulate) inv.setInventorySlotContents(slot, insertStack);
				return null;
			}
		}
