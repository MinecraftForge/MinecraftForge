        ...
        for (int i = 0; i < handler.getSlots(); i++)
        {
            ...
            if (extractItem != null)
            {
                for (int j = 0; j < dest.getSizeInventory(); j++)
                {
                    ...
                    if (destStack == null || destStack.stackSize < destStack.getMaxStackSize() && (dest instanceof TileEntityHopper && destStack.getMaxStatckSize() < ((TileEntityHopper)dest).getInventoryStackLimit()) && ItemHandlerHelper.canItemStacksStack(extractItem, destStack))
                    {
