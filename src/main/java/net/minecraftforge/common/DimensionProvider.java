...
public class DimensionProvider implements IForgeRegistryEntry<DimensionProvider>
{
    ...
    public static final IForgeRegistry<DimensionProvider> REGISTRY = ForgeRegistries.DIMENSIONS;

    static void init()
    {
        REGISTRY.register(new DimensionProvider(DimensionType.OVERWORLD, "minecraft:overworld").setDimIntID(0));
        REGISTRY.register(new DimensionProvider(DimensionType.NETHER, "minecraft:nether").setDimIntID(-1));
        REGISTRY.register(new DimensionProvider(DimensionType.THE_END, "minecraft:the_end").setDimIntID(1));
    }
     ...
     */
    public DimensionProvider(DimensionType type, String dimensionName)
    {
        this.type = type;
        this.ticksWaited = 0;
    ...
    private DimensionProvider(String dimensionName)
    {
        this.ticksWaited = 0;
        if (dimensionName.indexOf(":") == -1)
        {
    ...
    public final DimensionProvider setRegistryName(ResourceLocation dontUse)
    {
        return this;
    }
    ...
    public Class<DimensionProvider> getRegistryType()
    {
        return DimensionProvider.class;
    }
    ...
    public int getDimIntID()
    {
        if (dimIntID == -2)
        {
            setDimIntID(((ForgeRegistry<DimensionProvider>) REGISTRY).getID(dimID));
        }
        return dimIntID;
    }
    ...
    {
        DimensionProvider dim = REGISTRY.getValue(id);
     ...
     * @return the {@link DimensionProvider} (for chaining)
     */
    public static DimensionProvider dimensionWithCustomType(String dimensionName, String typeName, String typeSuffix, Class<? extends WorldProvider> provider,
            boolean keepLoaded)
    {
        DimensionProvider dim = new DimensionProvider(dimensionName);
        DimensionType type = DimensionType.register(typeName, typeSuffix, getDimIntID(dim.dimID), provider, keepLoaded);
        dim.setType(type);
        return dim;
    }

    private DimensionProvider setDimIntID(int dimIntID)
    {
