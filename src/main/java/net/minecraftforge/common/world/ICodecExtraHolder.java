...
{
    IdentityHashMap<ICodecExtraHolder, List<ICodecExtra>> EXTRA_HOLDER = new IdentityHashMap<>();

    default void setExtra(ICodecExtra extra)
    {
        List<ICodecExtra> extras = EXTRA_HOLDER.computeIfAbsent(this, s -> new ArrayList<>());
    ...
    {
        if(EXTRA_HOLDER.put(this, extras) != null)
        {
    ...
    {
        return EXTRA_HOLDER.get(this).stream().filter(e -> e.getType().equals(type)).findAny().map(e -> (A) e);
    }
    ...
    class ExtraParser<E extends ICodecExtraHolder> implements Codec<E>
    {
        ...
        public <T> DataResult<Pair<E, T>> decode(DynamicOps<T> ops, T input) {
            return this.base.decode(ops, input).flatMap(baseResult -> {
                DataResult<List<ICodecExtra>> extrasResult = ICodecExtra.LIST_EXTRAS.parse(ops, input);
        ...
        public <T> DataResult<T> encode(E input, DynamicOps<T> ops, T prefix) {
            return this.base.encode(input, ops, prefix)
                    .flatMap(t -> ICodecExtra.LIST_EXTRAS.encode(EXTRA_HOLDER.get(input), ops, t));
        }
    }
}
