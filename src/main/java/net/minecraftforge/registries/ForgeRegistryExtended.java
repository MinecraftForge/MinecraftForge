...
public class ForgeRegistryExtended<V extends IForgeRegistryEntry<V>> implements IForgeRegistryInternal<V>, IForgeRegistryModifiable<V>
{
    ...
    ForgeRegistryExtended(Class<V> superType, ResourceLocation defaultKey, int min, int max, @Nullable CreateCallback<V> create, @Nullable AddCallback<V> add, @Nullable ClearCallback<V> clear, RegistryManager stage, boolean allowOverrides, boolean isModifiable, @Nullable DummyFactory<V> dummyFactory, @Nullable MissingFactory<V> missing)
    {
        ...
        if(min<0)
        {
        	this.availabilityMap = new BitSet(Math.min(max-min+1, 0x0FFF-min));
        }
        else
        {
    ...
    public int getID(V value)
    {
        ...
        if(min<0)
        {
        return ret == null ? -1 +min: ret.intValue()+min;
        }
    ...
    ForgeRegistryExtended<V> copy(RegistryManager stage)
    {
        return new ForgeRegistryExtended<V>(superType, defaultKey, min, max, create, add, clear, stage, allowOverrides, isModifiable, dummyFactory, missing);
    }
    ...
    int add(int id, V value, String owner)
    {
        ...
        int idToUse = id;
        if (idToUse < 0+min || availabilityMap.get(idToUse))
        ...
        this.ids.put(idToUse-min, value);
        this.availabilityMap.set(idToUse-min);
    ...
    void sync(ResourceLocation name, ForgeRegistryExtended<V> from)
    {
        if (DEBUG)
    ...
    public V remove(ResourceLocation key)
    {
        ...
        if (value != null)
        {
            ...
            if (DEBUG)
                FMLLog.log.trace("Registry {} remove: {} {}", this.superType.getSimpleName(), key, id+min);
        }
    ...
    public void loadIds(Map<ResourceLocation, Integer> ids, Map<ResourceLocation, String> overrides, Map<ResourceLocation, Integer> missing, Map<ResourceLocation, Integer[]> remapped, ForgeRegistryExtended<V> old, ResourceLocation name)
    {
    	if(min<0)
    	{
    ...
    boolean markDummy(ResourceLocation key, int id)
    {
        ...
        this.availabilityMap.clear(id-min);
