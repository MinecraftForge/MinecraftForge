    ...
    static ArgumentBuilder<CommandSourceStack, ?> register()
    {
        return Commands.literal("dimensions")
            ...
            .executes(ctx -> {
                ...
                types.keySet().stream().sorted().forEach(key -> ctx.getSource().sendSuccess(Component.literal(key + ": " + types.get(key).stream().map(ResourceLocation::toString).sorted().collect(Collectors.joining(", "))), false));
                return 0;
            });
    }
}
