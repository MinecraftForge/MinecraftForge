...
public class ConfigTypesManager {
	...
	public static IConfigElement<?> makeConfigElement(final ModConfig modConfig, final String path, final Object obj) {
		if (obj instanceof ConfigValue) {
			...
			@Nullable final Object parentConfig;
			if (split.isEmpty())
...
//			return new InfoTextConfigElement<>("Uh, this is an error... " + path + ", " + obj);
	...
	@Nonnull
	public static <T> ConfigElementContainer<T> makeConfigElementContainer(final ModConfig modConfig, final ConfigValue<T> configValue) {
		return new ConfigElementContainer<>(configValue.getPath(), modConfig, configValue);
	}
	...
	public static <W extends Widget & IConfigListEntryWidget<?>> W makeListWidget(@Nonnull final List list, final int index, @Nonnull final ConfigScreen configScreen, @Nonnull final Predicate<Object> isValidPredicate, @Nonnull final Object obj) {
		...
		if (factory != null && factory != NO_FACTORY) {
			try {
				...
				return (W) factory.apply(makeScreenedCallback(configScreen, getter, setter, defaultValueGetter, isDefault, resetToDefault, isChanged, undoChanges, isValid));
			} catch (Exception e) {
				e.printStackTrace();
	...
	@Nonnull
	public static <T> ScreenedCallback<T> makeScreenedCallback(@Nonnull final ConfigScreen configScreen, final Supplier<T> getter, final Consumer<T> setter, final Supplier<T> defaultValueGetter, final BooleanSupplier isDefault, final Runnable resetToDefault, final BooleanSupplier isChanged, final Runnable undoChanges, final Predicate<Object> isValid) {
		return new ScreenedCallback<>(getter, setter, defaultValueGetter, isDefault, resetToDefault, isChanged, undoChanges, isValid, configScreen);
	}
	...
	public static Object getValueSpec(final ModConfig modConfig, final List<String> path) {
		...
		// Either a ValueSpec or a SimpleConfig (subcategory)
	...
	public static Object getConfigValue(final ModConfig modConfig, final List<String> path) {
		...
		// Either a ConfigValue or a SimpleConfig (subcategory)
	...
	public static Object getValue(final ModConfig modConfig, final List<String> path) {
		...
		// Either an Object (value) or a SimpleConfig (subcategory)
		Object ret = specConfigVales;

		for (final String s : path) {
			if (ret instanceof Map) // First iteration
				ret = ((Map<String, Object>) ret).get(s);
//			else if (ret instanceof SomethingOtherThanAConfig)
//				return ret; // Uh, shouldn't happen? TODO: Throw error?
			else if (ret instanceof Config)
				ret = ((Config) ret).get(s);
		}
