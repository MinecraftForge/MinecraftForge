...
import net.minecraftforge.fml.client.config.screen.ConfigEntryListWidget;
...
public class ElementConfigListEntry<T> extends ConfigListEntry<T> {
	...
	protected final ExtendedButton undoChangesButton, resetToDefaultButton;
	...
	public <W extends Widget & IConfigListEntryWidget<T>> ElementConfigListEntry(@Nonnull final ConfigScreen owningScreen, @Nonnull final W widget, final IConfigElement<T> configElement) {
		...
		this.children().add(this.resetToDefaultButton = new ExtendedButton(0, 0, 0, 0, RESET_CHAR, b -> this.resetToDefault()));
		...
		this.resetToDefaultButtonHoverChecker = new HoverChecker(this.resetToDefaultButton, ConfigScreen.HOVER_THRESHOLD);
	}
