...
//            FishingBobberEntity bob = (FishingBobberEntity) event.getEntity();
            ...
            if (EnchantmentHelper.getEnchantmentLevel(Register.GRAPPLE.get(), player)>0){
                if (trace.getType()!=RayTraceResult.Type.MISS){
                    ...
                    if (trace.getType()==RayTraceResult.Type.BLOCK) {//it does entities automatically
                        stickBobberToBlock(bob,trace.getLocation());
                    }
                }
            }
            ...
            if (punchLevel>0 && trace.getType()==RayTraceResult.Type.ENTITY){
                ...
                if (vector3d.lengthSqr() > 0.0D) {//copy of bow punch
...
//        }
    ...
    public static BlockPos getBlock(FishingBobberEntity bob) {
        ...
        if (bob.level.getBlockState(pos).isAir()) {//if bobber is in air (most of the time) we need to find a block
            double dx=tx-Math.floor(tx);
            double dy=ty-Math.floor(ty);
            double dz = tz - Math.floor(tz);
            if (Math.abs(dx - .5) > Math.abs(dy - .5)) {//x is closer than y
                if (Math.abs(dx - .5) > Math.abs(dz - .5)) {//x is closest
                    if (dx > .5) pos = new BlockPos(tx + 1.5 - dx, ty, tz);//middle of next block
                    else pos = new BlockPos(tx - .5 - dx, ty, tz);
                } else { //x closer than y, x farther than z
                    if (dz>.5) pos = new BlockPos(tx, ty, tz+1.5-dz);
                    else pos=new BlockPos(tx,ty,tz-.5-dz);
                }
            } else {//y closer than x
                if (Math.abs(dy - .5) > Math.abs(dz - .5)) {//y is closest
                    if (dy > .5) pos = new BlockPos(tx, ty + 1.5 - dy, tz);//middle of next block
                    else pos = new BlockPos(tx, ty - .5 - dy, tz);
                } else {//z is closest
                    if (dz>.5) pos = new BlockPos(tx, ty, tz+1.5-dz);
                    else pos=new BlockPos(tx,ty,tz-.5-dz);
                }
    ...
    public static void reelIn(EntityLeaveWorldEvent event) {
        if (!(event.getEntity() instanceof FishingBobberEntity)){return;}// && !event.getWorld().isClientSide) {
        ...
        if (bob.getPersistentData().contains("reel") && (hookedIn instanceof ArrowEntity) && !bob.level.isClientSide){
            ...
            if (arod.level>0) {
                ...
                if (!BlockReelEnchantment.canReel(bob.level,pos,arod.level)) return;
