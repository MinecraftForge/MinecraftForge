...
public class PlayerXpEventTest
{
    ...
    {
        if (!ENABLE) return;
        logger.info("{} picked up an experience orb worth {}", event.getPlayerEntity().getName().getString(), event.getOrbEntity().getXpValue());
    }
    ...
    {
        if (!ENABLE) return;
        logger.info("{} has been given {} experience", event.getPlayerEntity().getName().getString(), event.getAmount());
    }
    ...
    {
        if (!ENABLE) return;
        logger.info("{} has changed {} levels", event.getPlayerEntity().getName().getString(), event.getLevels());
    }
