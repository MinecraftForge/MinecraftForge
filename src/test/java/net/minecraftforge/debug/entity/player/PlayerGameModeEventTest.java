...
public class PlayerGameModeEventTest
{
    ...
    {
        if (!ENABLE) return;
        LOGGER.info("{} changed game mode. Current GameType: {}. New Game Type: {}", event.getPlayer(), event.getCurrentGameType(), event.getNewGameType());
        if (event.getNewGameType() == GameType.SURVIVAL)
    ...
    public static class PlayerGameModeEventTestClientForgeEvents
    {
        ...
        {
            if (!ENABLE) return;
            LOGGER.info("Client notified of changed game mode from '{}'. Current GameType: {}. New Game Type: {}", event.getPlayerInfo().getProfile(), event.getCurrentGameType(), event.getNewGameType());
        }
    }
}
