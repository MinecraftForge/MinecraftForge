...
public class ShieldBlockTest
{
    ...
    {
        if (event.getDamageSource().getImmediateSource() instanceof AbstractArrowEntity && event.getEntityLiving() instanceof PlayerEntity)
        {
            ((PlayerEntity) event.getEntityLiving()).inventory.addItemStackToInventory(new ItemStack(Items.ARROW));
            event.setBlockedDamage(event.getOriginalBlockedDamage() / 2);
            event.getDamageSource().getImmediateSource().remove();
        }
    }
}