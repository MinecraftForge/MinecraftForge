...
public class ShieldBlockTest
{
    ...
    {
        if (event.getDamageSource().getDirectEntity() instanceof AbstractArrowEntity && event.getEntityLiving() instanceof PlayerEntity)
        {
            ((PlayerEntity) event.getEntityLiving()).inventory.add(new ItemStack(Items.ARROW));
            event.setBlockedDamage(event.getOriginalBlockedDamage() / 2);
            event.getDamageSource().getDirectEntity().remove();
        }
    }
}