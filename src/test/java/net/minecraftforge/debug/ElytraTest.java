...
public class ElytraTest
{
	...
	{
		ELYTRA_BOOTS = new ItemArmor(ArmorMaterial.LEATHER, 0, EntityEquipmentSlot.FEET).setUnlocalizedName("elytra_boots")
				.setRegistryName(new ResourceLocation("elytratest","elytra_boots")).setCreativeTab(CreativeTabs.COMBAT);
	...
	public void onPlayerTick(PlayerTickEvent event)
	{
		if (event.phase == Phase.START)
		{
			...
			IAttributeInstance attributeElytraFlight = event.player.getAttributeMap().getAttributeInstance(SharedMonsterAttributes.ELYTRA_FLIGHT);
			...
			if (attributeElytraFlight != null)
			{
				...
				if (hasBoots)
				{
					attributeElytraFlight.applyModifier(new AttributeModifier(UUID_ELYTRA_BOOTS_FLIGHT, "ElytraFlightBoots", 1.0D, 0));
				}
			}
	...
	{
		event.getRegistry().register(new PotionType(new PotionEffect(ANTI_ELYTRA_POTION, 300)).setRegistryName("elytratest", "antieyltrapotion"));
	}
    ...
    {
        ModelLoader.setCustomModelResourceLocation(ELYTRA_BOOTS, 0, new ModelResourceLocation("minecraft:leather_boots", "inventory"));
    }
}
