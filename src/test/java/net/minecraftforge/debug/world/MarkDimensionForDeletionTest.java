        ...
        public ActionResult<ItemStack> onItemRightClick(World worldIn, PlayerEntity playerIn, Hand handIn)
        {
            if (!worldIn.isRemote)
            {
                ...
                else
                {
                    ...
                    {
                        ResourceLocation dimName = new ResourceLocation(MODID, "dynamic_" + new Random().nextInt(51));
                        ...
                        playerIn.sendStatusMessage(new StringTextComponent("Dimension: " + dimName.toString()), true);
                    }
                    else if (playerIn.dimension == cap.dimension)
                    {
                        playerIn.changeDimension(DimensionType.OVERWORLD, teleporter);
                        cap.dimension = null;
                        playerIn.sendStatusMessage(new StringTextComponent("Dimension: " + DimensionType.OVERWORLD.getRegistryName().toString()), true);
                    }
                    else
                    {
                        playerIn.changeDimension(cap.dimension, teleporter);
