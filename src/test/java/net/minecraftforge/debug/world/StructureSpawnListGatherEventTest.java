...
public class StructureSpawnListGatherEventTest
{
    ...
    public StructureSpawnListGatherEventTest() {
            MinecraftForge.EVENT_BUS.addListener(this::onStructureSpawnListGather);
    }
    ...
    private void onStructureSpawnListGather(StructureSpawnListGatherEvent event) {
        if (event.getStructure() == Structure.field_236375_k_) {
            event.addEntityClassification(EntityClassification.MONSTER);
            event.getEntitySpawns().get(EntityClassification.MONSTER).add(new MobSpawnInfo.Spawners(EntityType.WITHER_SKELETON, 100, 5, 15));
            LOGGER.info("Adding wither skeleton spawns to strong holds");
        } else if (event.getStructure() == Structure.field_236373_i_) {
            event.checkOutsideStructurePieces();
            event.addEntityClassification(EntityClassification.MONSTER);
            event.getEntitySpawns().get(EntityClassification.MONSTER).add(new MobSpawnInfo.Spawners(EntityType.GUARDIAN, 100, 1, 5));
