...
public class DeferredRegistryTest {
    ...
    {
        event.getRegistry().register(new BlockItem(BLOCK.get(), new Item.Properties().group(ItemGroup.BUILDING_BLOCKS)).setRegistryName(BLOCK.get().getRegistryName()));
    }
        ...
        if (event.includeClient()) {
            gen.addProvider(new BlockStateProvider(gen, MODID, event.getExistingFileHelper()) {
                ...
                protected void registerStatesAndModels() {
                    ModelFile model = cubeAll(BLOCK.get().getRegistryName().getPath(), mcLoc("block/furnace_top"));
