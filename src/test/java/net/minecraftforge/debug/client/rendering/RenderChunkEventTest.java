...
public class RenderChunkEventTest
{
    ...
    {
        VANILLA, MODDED_VANILLA_BEHAVIOUR, MODDED_VANILLA_CODE_READABILITY_BEHAVIOUR, UPSIDE_DOWN, MARCHING_CUBES
    }

    static RenderType[] renderTypes = new RenderType[4];

    @SubscribeEvent
    public static void renderChunkVanilla(final RebuildChunkBlocksEvent event)
    {

        renderTypes = new RenderType[] {
                RenderType.VANILLA, RenderType.MODDED_VANILLA_BEHAVIOUR, RenderType.MARCHING_CUBES
        };

        if ((renderTypes == null) || (renderTypes.length <= 0) || ((renderTypes.length == 1) && (renderTypes[0] == RenderType.VANILLA)))
        {
    ...
    @SubscribeEvent
    public static void renderChunkModdedVanilla(final RebuildChunkBlocksEvent event)
    {

        try
        {

            if (!Arrays.asList(renderTypes).contains(RenderType.MODDED_VANILLA_BEHAVIOUR))
            {
                return;
            }
            ...
            for (final BlockPos.MutableBlockPos blockpos$mutableblockpos : event.getChunkBlockPositions())
            {
                ...
                for (final BlockRenderLayer blockrenderlayer1 : BlockRenderLayer.values())
                {
                    ...
                    if (block.getDefaultState().getRenderType() != EnumBlockRenderType.INVISIBLE)
                    {
                        ...
                        isBlockRenderLayerActive[j] |= blockRendererDispatcher.renderBlock(state, blockpos$mutableblockpos, event.getWorldView(), bufferbuilder);
                    }
                }
    ...
    public static void renderChunkMarchingCubes(final RebuildChunkBlocksEvent event)
    {
        ...
        try
        {
            ...
            if (!Arrays.asList(renderTypes).contains(RenderType.MARCHING_CUBES))
            {
                return;
            }
