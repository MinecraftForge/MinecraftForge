...
public class InfiBucketWood extends Item
    implements ITextureProvider
{
    private int isFull;

    public InfiBucketWood(int i, int j)
    {
        super(i);
        maxStackSize = 1;
        isFull = j;
    ...
    public ItemStack onItemRightClick(ItemStack itemstack, World world, EntityPlayer entityplayer)
    {
        ...
        if (movingobjectposition.typeOfHit == EnumMovingObjectType.TILE)
        {
            ...
            if (isFull == 0)
            {
                		...
                		&& world.getBlockMetadata(i, j, k) == 0)
                {
                    ...
                    return new ItemStack(mod_InfiTools.woodBucketWater);
                }
                if (bID == Block.sand.blockID)
                {
                    world.setBlockWithNotify(i, j, k, 0);
                    return new ItemStack(mod_InfiTools.woodBucketSand);
                }
                if (bID == Block.gravel.blockID)
                {
                    world.setBlockWithNotify(i, j, k, 0);
                    return new ItemStack(mod_InfiTools.woodBucketGravel);
                }
            }
            else
            {
                if (isFull < 0)
                {
                    return new ItemStack(mod_InfiTools.woodBucketEmpty);
                }
        ...
        {
            return new ItemStack(mod_InfiTools.woodBucketMilk);
        }
        return itemstack;
    }
