    ...
    public ItemStack onItemRightClick(ItemStack itemstack, World world, EntityPlayer entityplayer)
    {
        ...
        if (movingobjectposition.typeOfHit == EnumMovingObjectType.TILE)
        {
            ...
            if (isFull == 0)
            {
                ...
                {
                    world.setBlockWithNotify(i, j, k, 0);
                    return new ItemStack(mod_InfiTools.iceBucketSand);
                }
                if (bID == Block.gravel.blockID)
                {
                    world.setBlockWithNotify(i, j, k, 0);
                    return new ItemStack(mod_InfiTools.iceBucketGravel);
                }
            }
            else
            {
                if (isFull < 0)
                {
                    return new ItemStack(mod_InfiTools.iceBucketEmpty);
                }
        ...
        {
            return new ItemStack(mod_InfiTools.iceBucketMilk);
        }
        return itemstack;
    }
