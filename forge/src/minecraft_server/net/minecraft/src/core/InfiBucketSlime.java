    ...
    implements ITextureProvider
{
    ...
    public ItemStack onItemRightClick(ItemStack itemstack, World world, EntityPlayer entityplayer)
    {
        ...
        if (movingobjectposition.typeOfHit == EnumMovingObjectType.TILE)
        {
            ...
            if (isFull == 0)
            {
                		...
                		&& world.getBlockMetadata(i, j, k) == 0)
                {
                    ...
                    return new ItemStack(mod_InfiTools.slimeBucketWater);
                }
                if (bID == Block.sand.blockID)
                {
                    world.setBlockWithNotify(i, j, k, 0);
                    return new ItemStack(mod_InfiTools.slimeBucketSand);
                }
                if (bID == Block.gravel.blockID)
                {
                    world.setBlockWithNotify(i, j, k, 0);
                    return new ItemStack(mod_InfiTools.slimeBucketGravel);
                }
            }
            else
            {
                if (isFull < 0)
                {
                    return new ItemStack(mod_InfiTools.slimeBucketEmpty);
                }
        ...
        {
            return new ItemStack(mod_InfiTools.slimeBucketMilk);
        }
        return itemstack;
    }
