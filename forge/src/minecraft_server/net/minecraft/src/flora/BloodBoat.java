...
public class BloodBoat extends Entity
{
    ...
    public BloodBoat(World world, double d, double d1, double d2)
    {
        this(world);
    ...
    public void onUpdate()
    {
        ...
        for (int j = 0; j < i; j++)
        {
            ...
            if (worldObj.isAABBInMaterial(axisalignedbb, Material.water) ||
            		worldObj.isAABBInMaterial(axisalignedbb, Material.lava))
            {
                d += 1.0D / (double)i;
            }
        }
        ...
        if (isCollidedHorizontally && d1 > 1D)
        {
            if (!worldObj.isRemote)
            {
                setEntityDead();
        ...
        if (list != null && list.size() > 0)
        {
            for (int j1 = 0; j1 < list.size(); j1++)
            {
                ...
                if (entity != riddenByEntity && entity.canBePushed() && (entity instanceof BloodBoat))
                {
