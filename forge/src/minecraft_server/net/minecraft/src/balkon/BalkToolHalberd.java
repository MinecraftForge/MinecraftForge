...
public class BalkToolHalberd extends InfiWeaponBase
    implements ITextureProvider
{
    ...
    public BalkToolHalberd(int i, int j, int k, int l, int i1)
    {
        super(i, 2, 3, j, 15F, k, true, materialEffectiveAgainst, l, i1);
        weaponDamage = 5 + k * 2;
        initialWeaponDamage = 5 + k * 2;
        stabMode = false;
        knockback = 1.5F;
        initialKnockback = 1.5F;
    }
    ...
    public ItemStack onItemRightClick(ItemStack itemstack, World world, EntityPlayer entityplayer)
    {
        ...
        if (stabMode)
        {
            knockback = initialKnockback * 0.6F;
            weaponDamage = (int)((double)initialWeaponDamage * 1.2D);
        }
        else
        {
            knockback = initialKnockback;
    ...
    {
        return "/infibalkon/balkhalberds.png";
    }
