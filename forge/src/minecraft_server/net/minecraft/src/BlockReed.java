...
public class BlockReed extends Block
{
    ...
    public boolean canPlaceBlockAt(World world, int x, int y, int z)
    {
        ...
        if (bID != Block.grass.blockID && bID != Block.dirt.blockID && bID != Block.sand.blockID)
        {
            return false;
        }
        else
        {
            return isWaterNearby(world, x, y - 1, z);
        }
    }
    ...
    private boolean isWaterNearby(World world, int i, int j, int k)
    {
        ...
        {
            for (int i1 = k - 3; i1 <= k + 3; i1++)
            {
                if (world.getBlockMaterial(l, j, i1) == Material.water)
                {
                    return true;
                }
            }
