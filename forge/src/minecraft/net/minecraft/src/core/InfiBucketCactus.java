...
public class InfiBucketCactus extends Item
    implements ITextureProvider
{
    private int isFull;

    public InfiBucketCactus(int i, int j)
    {
        super(i);
        maxStackSize = 1;
        isFull = j;
    ...
    public ItemStack onItemRightClick(ItemStack itemstack, World world, EntityPlayer entityplayer)
    {
        ...
        if (movingobjectposition.typeOfHit == EnumMovingObjectType.TILE)
        {
            ...
            if (isFull == 0)
            {
                		...
                		&& world.getBlockMetadata(i, j, k) == 0)
                {
                    ...
                    return new ItemStack(mod_InfiTools.cactusBucketWater);
                }
                if (bID == Block.sand.blockID)
                {
                    world.setBlockWithNotify(i, j, k, 0);
                    return new ItemStack(mod_InfiTools.cactusBucketSand);
                }
                if (bID == Block.gravel.blockID)
                {
                    world.setBlockWithNotify(i, j, k, 0);
                    return new ItemStack(mod_InfiTools.cactusBucketGravel);
                }
            }
            else
            {
        ...
        {
            return new ItemStack(mod_InfiTools.cactusBucketMilk);
        }
        return itemstack;
    }
