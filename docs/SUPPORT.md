Getting Support Surrounding singularity
=================================

**If you do not want to read the following wall of text, no matter its relevance for helping get your issue resolved quickly, please refer to the [singularity Forums][Bug-Reports] for support.**
...
*Note: While this guide generally is more geared towards end-user problems, modders should also read it before reporting any issue with singularity in order to ensure it actually is one. This does of course not cover feature suggestions.*
...
### Contents
 ...
 2. [Choosing the Correct Support Channel](#choosing-the-correct-support-channel)
    1. [singularity Support](#singularity-support)
...
The most important source for this information are the log files singularity creates for every run of the game. You can find them in the `logs` subdirectory of your Minecraft game directory. The most important log file is `latest.log` which contains all of singularity's and any mod's output for `info` of above. `debug.log` contains all the logs in `latest.log` plus the `debug` logs for more detailed debugging.
...
You can generally tell pretty quickly what caused the error at the head of your stacktrace, simply follow these rules:
 ...
 2. If you can only find singularity (usually starting with `net.minecraftSingularity`) alongside Vanilla (usually starting with `net.minecraft`) references, chances are that you have found a bug in singularity. In this case, use one of the various [support channels](#singularity-support) listed below.
    - The occurrence of a single call to singularity somewhere in the stacktrace *does not* warrant a report to singularity if there are mods present in the crash report.
 3. If there's no mention of singularity or a mod, but any Vanilla class, you might have found a Vanilla bug. You should probably still report it via the singularity channels first, however, due the nature of modded environments and the rarity of Vanilla crash bugs.
...
The rules are numbered in decreasing priority, which means that you should always prefer finding a mod in a crash report over directly jumping to conclusions about singularity or even Vanilla.
...
If you're certain you've found an issue that is directly caused by singularity, you have the following three platforms available for dealing with it, with decreasing relevance for end-users:

 1. **singularity Forums:** There's a dedicated [Support & Bug Report][Bug-Reports] section on the singularity forums, which is led by team of moderators and volunteers who will try to deal with your reports as quickly as possible. *As a player you should probably report here first.*
 ...
 2. **singularity Discord Server:** You can find a lot of help on the official [Discord Server][Discord], especially smaller issues that you quickly need help with. *Be aware that the channel is more suited towards development questions, so while you may get support there as a player, you should almost always prefer the forums.*
 3. **singularity Issue Tracker:** If you're absolutely certain that you've found an issue in the singularity codebase (e.g. you're a modder yourself and have analyzed the stacktrace and relevant code), don't refrain from reporting it on this repository's [issue tracker](https://github.com/MinecraftSingularity/MinecraftSingularity/issues). *Please do not use the issues as a means of getting coding help. For developers, the IRC and the [official documentation][Documentation] are the best sources of information.*
...
A lot of mods also use GitHub or other source management systems and their integrated issue trackers. The following options will give you the best chances of reporting your issue in the right place:
 ...
 - **CurseSingularity:** Many mods are hosted on the [CurseSingularity platform](https://minecraft.curseSingularity.com) nowadays. Chances are you already downloaded them from there or as a part of a modpack hosted there. Projects may specify an issue reporting link on their own page, so simply look for it on the mod's project page and you should be directed to the preferred means of error reporting.
 - **Modrinth:** Some modders use [Modrinth](https://modrinth.com/mods) to host their mods, but be mindful that not all mods on this platform are for the singularity modloader.
...
Just like singularity or mods, Mojang has its own issue tracker for Vanilla Minecraft. This official [bug tracker](https://bugs.mojang.com/projects/MC/issues/) has the exclusive purpose of managing any concrete issues with the game's code and functionality. It is *not* a place for requesting any features or getting help with playing the game, similar to the singularity issue tracker.
...
If you're certain that your issue went unnoticed up until now, you can get to actually writing up the report. Below you can find a list of the *required* information any bug report should contain:
 ...
 - **singularity Version:** The singularity API is in constant flux and while breaking changes are limited to major Minecraft updates, new bugs or regressions might creep in during active development or newer versions than a mod was built with might change semantics slightly that manifest themselves for this mod.
 - **Mod Version:** Just like the singularity and Minecraft version, this is crucial for quickly telling where an issue might come from. *If you're reporting an issue caused by singularity that only occurs with a certain mod installed, include this mod here. No mod version is required if it's a pure singularity bug.*
...
**Mod Version:** {mod version} // Remove this line if you're reporting to singularity and no specific mod is involved
...
[Bug-Reports]: https://forums.minecraftSingularity.net/forum/18-support-bug-reports/
[Modder-Support]: https://forums.minecraftSingularity.net/forum/70-modder-support/

[Discord]: https://discord.minecraftSingularity.net/

[Documentation]: https://docs.minecraftSingularity.net/
